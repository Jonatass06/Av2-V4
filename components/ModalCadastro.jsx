...
export default function ModalCadastro({ post, fechar }) {
    ...
    return (
        <div onClick={e => e.target.id == "fundo" && fechar()} id="fundo"
            ...
            <form className="w-[700px] h-[650px] bg-white shadow-10b ronded-md flex flex-col justify-center items-center gap-6 relative ">
                ...
                <button className="bg-verde w-[370px] h-[80px] text-branco text-[24px] font-alata rounded-[5px]" onClick={() => cadastrar()}>Continuar</button>
            </form>
           <Erro consdicao={erro} fechar={() => setErro(false)} mensagem={"Já existe um usuário com esse nome cadastrado!"}/>
        </div>
